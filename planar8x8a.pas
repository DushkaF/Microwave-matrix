...
uses
  ...
  Math, DateUtils,
  StdCtrls, Menus, PairSplitter, Spin, ValEdit, Grids, TAGraph,
...
var
  ...
  //Chart: array[1..PlotsNumX, 1..PlotsNumY] of TChart;
  Chart: array[1..PlotsNumMax] of TChart;

  //ChartLineSeries:  array[1..PlotsNumX, 1..PlotsNumY] of TLineSeries;
  ...
  DataRe, DataIm, Data, CalibrationDataRe, CalibrationDataIm: array[1..PlotsNumMax, 1..PlotSeriesNumMax, 0..DataSizeMax-1] of Single;
...
Chart[ii].BottomAxis.LabelSize:=22;
...
begin
  DataRe[NumPlot, NumSer, ii] := StrToFloat(StringList.Strings[2*ii]);
  //DebugLn(DataRe[1, ii]);
  DataIm[NumPlot, NumSer, ii] := StrToFloat(StringList.Strings[2*ii+1]);

  if (PlotParams[NumPlot].SFormat = 0) //SFormat 0-mlog, 1-phase
  then Data[NumPlot, NumSer, ii] := 20*Log10( hypot( DataRe[NumPlot, NumSer, ii], DataIm[NumPlot, NumSer, ii] ) )
  else Data[NumPlot, NumSer, ii] := RadToDeg( ArcTan2( DataIm[NumPlot, NumSer, ii], DataRe[NumPlot, NumSer, ii] ) );
...
begin
  ...
  PlotParams[Params.CurrentPlotNum].SP[Params.CurrentSeriesNum].SName:=RadioGroupS11S21.ItemIndex;
...
begin
  PlotParams[Params.CurrentPlotNum].SP[Params.CurrentSeriesNum].SwitchParams.InputNum := RadioGroupSwitchInNum.ItemIndex;
end;
...
begin
  PlotParams[Params.CurrentPlotNum].SP[Params.CurrentSeriesNum].SwitchParams.OutputNum := RadioGroupSwitchOutNum.ItemIndex;
end;
...
//set gui cotrols
RadioGroupS11S21.ItemIndex := PlotParams[Params.CurrentPlotNum].SP[Params.CurrentSeriesNum].SName;
RadioGroupMagPhase.ItemIndex := PlotParams[Params.CurrentPlotNum].SFormat;
CheckBoxCalibrationApply.Checked := PlotParams[Params.CurrentPlotNum].SP[Params.CurrentSeriesNum].CalibrationApply;
RadioGroupSwitchInNum.ItemIndex := PlotParams[Params.CurrentPlotNum].SP[Params.CurrentSeriesNum].SwitchParams.InputNum;
RadioGroupSwitchOutNum.ItemIndex := PlotParams[Params.CurrentPlotNum].SP[Params.CurrentSeriesNum].SwitchParams.OutputNum;
...
if PlotParams[NumPlot].SP[jj].CalibrationApply = False  //no calibration
then
  ChartLineSeries[NumPlot, jj].AddXY(Params.FStart+ii*FStep, Data[NumPlot, jj, ii]-2*jj)  //even indexes
...
if ( (MarkerParams[ii].MarkerEnabled = True)
      and (MarkerParams[ii].MarkerFrequency > Params.FStart) and (MarkerParams[ii].MarkerFrequency < Params.FStop) )
then
begin
...
case PlotParams[Params.PlotsNumStart+ii].SP[jj].SName of
//case jj-1 of
0 : //none
begin
...
if PlotParams[Params.PlotsNumStart+ii].SP[jj].SName=1 //s11
then
begin
  //c.SendString ('CALC:PAR:DEF S11'#10);
  //sleep(dt);
...
if PlotParams[Params.PlotsNumStart+ii].SFormat=0  //mlog
then
begin
...
try
  ...
  if PlotParams[Params.CurrentPlotNum].SP[Params.CurrentSeriesNum].CalibrationApply then
    CalStatus := 'ON'
  else
    CalStatus := 'OFF';
  ...
  Writeln(F, Format('!Switch from port %d to port %d', [PlotParams[Params.CurrentPlotNum].SP[Params.CurrentSeriesNum].SwitchParams.InputNum + 1, PlotParams[Params.CurrentPlotNum].SP[Params.CurrentSeriesNum].SwitchParams.OutputNum + 1]));
  Writeln(F, '!' + TimeStamp);
  // Запись заголовка
  case SaveFormat of
...
case PlotParams[PlotIndex].SP[1].SName of
...
for ii:=1 to PlotsNumMax do
begin
  PlotParams[ii].SP[1].SName:=1; //s11=0,s21=1
...
begin
  PlotParams[Params.CurrentPlotNum].SP[Params.CurrentSeriesNum].CalibrationApply := CheckBoxCalibrationApply.Checked;
...
begin
 ...
 DebugLn(['SpinEdit - MarkerParams[MarkerNum].MarkerChartNum = ', MarkerParams[MarkerNum].MarkerNum]);
end;
